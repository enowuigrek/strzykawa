...
export function TimelineSection({ year, title, content, images = [], index }) {
    ...
    return (
        <section id={`year-${year}`} className="scroll-mt-32" ref={ref}>
            {/* Desktop: Main Content + Gallery */}
            <div
                className={`grid md:grid-cols-5 gap-10 md:gap-16 items-stretch ${
                    isEven ? '' : 'md:grid-flow-dense'
                }`}
            >
                {/* Content - Full height with animation at bottom */}
                <div
                    className={`md:col-span-2 ${isEven ? '' : 'md:col-start-4'} transition-all duration-700 ease-out ${
                        isVisible
                ...
                >
                    <div className="h-full flex flex-col bg-gradient-to-r from-primary-light/20 to-primary/30 border border-white/10 p-6 md:p-8 space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <h3 className="text-3xl md:text-4xl text-white">{title}</h3>
                        </div>
                ...
                {images.length > 0 && (
                    <div
                        className={`md:col-span-3 ${isEven ? '' : 'md:col-start-1 md:row-start-1'} transition-all duration-700 ease-out delay-100 ${
                            isVisible
