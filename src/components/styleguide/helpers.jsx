...
export function SpacingRow({ size, px, usage, highlight }) {
    return (
        <div
            className={`flex items-center gap-4 p-3 ${highlight ? 'bg-accent/10 border border-accent/30' : 'bg-primary-light'}`}
        >
...
export function ZIndexCard({ level, usage, highlight }) {
    return (
        <div
            className={`flex items-center justify-between p-4 border ${highlight ? 'border-success/50 bg-success/10' : 'border-accent/30 bg-primary-light'}`}
        >
...
export function TimingRow({ name, value, usage, unit = 'ms', highlight }) {
    return (
        <div
            className={`flex items-center justify-between p-3 ${highlight ? 'bg-success/10 border border-success/30' : 'bg-primary-light border border-accent/30'}`}
        >
